...
predicate different(v: array<int>) 
  reads v;
  { forall i: nat, j: nat :: i <= j < v.Length ==> v[i] != v[j] }
...
predicate slipped(v: array<int>, w: array<int>, k: int)
  ...
  reads v, w
  { forall i: nat :: i < v.Length ==> v[i] == w[ (k + i)%v.Length ] }
...
lemma firstMustAppear(v: array<int>, w: array<int>)
 ...
 // P => Q <==> !Q => !P
  {
    calc == {
      !(forall k :: 0 <= k < v.Length ==> !slipped(v, w, k));
      (exists k :: 0 <= k < v.Length && slipped(v, w, k));
      (exists k :: 0 <= k < v.Length && v[0] == w[ (k + 0)%v.Length ]);
      !(forall k :: 0 <= k < v.Length ==> v[0] != w[k]);
    }
  }
...
method mslipped(v: array<int>, w: array<int>) returns (b : bool,i:int)
  ...
  ensures b <==> slipped(v, w, i)
  ensures !b ==> i == v.Length
  {
    b := true;
    i := 0;
    while(i < v.Length && b)
...
/*

  {
    ...
    invariant b <==> 0 <= i < v.Length && slipped(v, w, i)
    invariant !b ==> i == v.Length  
    {
      b := (v[0] != w[i]);
      i := i + 1;
