...
\end{code}
 ...
 --Exercise 2': Confirm that the error vanishes once you
 --fix a type for halve (Exercise 2)
...
-- Exercise 4: complete the implementation and prove the refined type
...
-- Exercise 5: give filter' a refined type 
...
--{-@ reverse    :: xs:UList a -> {v:UList a | len xs > 0 ==> len v > 0}    @-}
reverse         = go []
  where
    {-@ go     :: acc:[a] -> xs:[a] -> [a] @-}
    go acc []     = acc
...
-- Exercise 6: give go a refined type 
...
-- Exercise 7: refine the type of append to ensure a unique list as output 
...
assert :: Bool -> p -> p
assert True  x = x
assert False _ = die "yikes, assertion fails!"

focusLeft                 :: Ord a => Zipper a -> Zipper a
focusLeft (Zipper t (l:ls) rs) = Zipper l ls (t:rs)
focusLeft (Zipper t [] rs)     = --assert (not (isin x xs) && unique (xs)) $
                                 Zipper x xs []
  where  
    (x:xs) = reverse (t:rs)
\end{code}
